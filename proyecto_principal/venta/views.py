...
def ventas(request):
    ...
    if producto_nombre:
        ventas_qs = ventas_qs.filter(
            Q(detalles__producto__nombre__icontains=producto_nombre) |
            Q(detalles__producto__codigo__codigo__icontains=producto_nombre)
        ).distinct()
    ...
    for venta in lista_ventas:
        ...
        for d in venta.detalles.select_related('producto').filter(cantidad__gt=0):
    ...
    try:
        with transaction.atomic():
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    'success': True,
                    'message': f'Venta #{venta.id_venta} creada exitosamente.'
                })
...
def autocomplete_productos(request):
    ...
    producto_exacto = productos.filter(
    ...
    # Limitar a 5 resultados y ordenar por nombre
    productos = productos.order_by('nombre')[:5]
...
def eliminar_historial_ventas(request):
    ...
    if request.method == 'POST':
        ...
        if cantidad == 'all':
            ...
            messages.success(request, '¡Historial de ventas eliminado correctamente!')
        else:
            try:
                ...
                messages.success(request, f'¡Se eliminaron las últimas {n} ventas correctamente!')
            except Exception:
